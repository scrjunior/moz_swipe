...
export default function CriativoDetailPage() {
  ...
  // Get file extension from criativo title or drive link
  ...
  const handleDownload = async () => {
    ...
    if (isGoogleDriveVideo(criativo.drive_link)) {
      const fileId = extractFileId(criativo.drive_link);
      if (!fileId) {
      ...
      try {
        ...
        try {
          ...
          const fileExtension = getFileExtension(criativo.title, criativo.drive_link);
          const fileName = criativo.title.includes('.') ? criativo.title : `${criativo.title}.${fileExtension}`;
      ...
      window.open(criativo.drive_link, '_blank');
      setTimeout(() => setDownloadStatus(""), 2000);
    }
  };

  // Load profile + criativo
  useEffect(() => {
    const fetchProfileAndCriativo = async () => {
      try {
        setLoading(true);
        ...
        if (params.id) {
          const { data: criativoData, error: criativoError } = await supabase
            .from("criativos")
            .select(`
              *,
          ...
          if (criativoError) {
            console.error("Database error:", criativoError);
            setError(`Erro na base de dados: ${criativoError.message}`);
            setLoading(false);
            return;
          }

          if (!criativoData) {
            setError("Criativo não encontrado");
            setLoading(false);
            return;
          }

          const transformedCriativo = {
            ...criativoData,
            oferta: criativoData.contents || null
          };

          setCriativo(transformedCriativo);

          if (transformedCriativo.oferta?.id) {
            const { data: relatedData, error: relatedError } = await supabase
              ...
              `)
              .eq("oferta_id", transformedCriativo.oferta.id)
              .neq("id", params.id)
              .limit(8);
            ...
            if (!relatedError && relatedData) {
              ...
              setRelatedCriativos(transformedRelated);
            }
          }

          // Fetch related landing pages for this criativo
          const { data: pagesData, error: pagesError } = await supabase
            .from("landing_pages")
            .select("id, title, page_url, association_type")
            .eq("criativo_id", params.id)
            .eq("association_type", "criativo");
        ...
        console.error(err);
        setError("Erro ao carregar criativo");
        setLoading(false);
      }
    };

    fetchProfileAndCriativo();
    ...
    return (
      <div className="flex min-h-screen bg-[#0f0f0f] text-white items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-lg mb-4">{error || "Criativo não encontrado"}</p>
          <button
            onClick={() => router.back()}
    ...
    <div className="flex min-h-screen bg-[#0f0f0f] text-white">
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-col gap-1 ml-6">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-white font-bold text-left hover:bg-[#1f1f1f]"
                >
                  <Image src="/icons/paginas.svg" alt="Paginas" width={22} height={18} />
                  Páginas
                </button>
              </div>
            </div>
      ...
      <main className="flex-1 px-8 pb-8 overflow-auto min-w-0">
        ...
        <div className="mt-6 max-w-6xl mx-auto px-1">
          {/* Player box - RESPONSIVO */}
          {isGoogleDriveVideo(criativo.drive_link) && (
            <div className="mb-6 relative bg-neutral-800 rounded-lg p-4">
              <div className="relative w-full overflow-hidden rounded-lg" style={{ paddingBottom: "56.25%" }}>
                <iframe
                  src={getPreviewUrl(criativo.drive_link) || ""}
          ...
          {/* Link to related oferta */}
          {criativo.oferta && (
            <div className="mb-6 p-4 bg-[#141414] rounded-lg flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm mb-1">Oferta Relacionada</p>
                <p className="text-white font-semibold">{criativo.oferta.title}</p>
              </div>
              <button
                onClick={() => router.push(`/content/${criativo.oferta?.id}`)}
          ...
          {relatedCriativos.length > 0 && (
            <div className="mt-8">
              <h2 className="text-xl font-bold mb-4">Outros Criativos desta Oferta</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {relatedCriativos.map((relatedCriativo) => (
                  <div
                    key={relatedCriativo.id}
                    ...
                    onClick={() => handleCriativoClick(relatedCriativo)}
                  >
                    <div className="relative">
                      <img
                        src={
                          relatedCriativo.oferta?.thumbnail
                            ? getOptimizedImageUrl(relatedCriativo.oferta.thumbnail)
                            : "/default-creative.jpg"
                        }
                        alt={relatedCriativo.title}
                    ...
                    <div className="p-4">
                      <h3 className="font-semibold text-white mb-1 line-clamp-2 group-hover:text-blue-300 transition-colors">
                        {relatedCriativo.title}
                      </h3>
                      <p className="text-gray-400 text-sm">
                        {relatedCriativo.oferta?.title || "Oferta não encontrada"}
                      </p>
                    </div>
                  </div>
                ))}
